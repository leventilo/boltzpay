...
Available categories: `crypto-data`, `finance`, `research`, `utilities`, `dev-tools`, `ai-tools`, `social`, `demo`, `media`.
...
<Info>
  Read-only commands (`check`, `quote`, `discover`, `diagnose`, `budget`, `history`, `wallet`) work without Coinbase credentials. Only `fetch` requires them. See [Configuration](/getting-started/configuration) for constructor options, `.env` examples, and budget details.
</Info>
