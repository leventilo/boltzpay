...
The BoltzPay MCP server gives Claude 8 tools for discovering, quoting, diagnosing, and paying for API data, all through natural language. It communicates via the [Model Context Protocol](https://modelcontextprotocol.io/), an open standard for AI tool use.
...
<Steps>
  ...
  <Step title="Add the BoltzPay server (Explore mode, no keys needed)">
    ...
    This gives Claude read-only tools: `check`, `quote`, `discover`, `diagnose`, `budget`, `history`, and `wallet`. No credentials required.
  </Step>
...
The `boltzpay_fetch` tool requires Coinbase keys to make payments. Read-only tools (`check`, `quote`, `discover`, `diagnose`) work without credentials. To enable payments, add the three `COINBASE_*` env vars to your config.
