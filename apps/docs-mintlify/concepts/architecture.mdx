...
A terminal interface that wraps the SDK. Commands: `quote`, `fetch`, `discover`, `diagnose`, `budget`, `history`, `wallet`, `demo`, `verify-directory`. The `discover` command supports `--protocol`, `--min-score`, and `--query` flags for filtered searches. Also serves as a bridge for Python agents (LangChain, CrewAI) via subprocess calls.
